...
select round(avg(fiyat)) from arac;--Kusurati yazmadi

select round(avg(fiyat),2) from arac;--Virgulden sonra iki degeri getirdi.
...
--meslekler tablosunda ilk 2 veriden sonra 5 veriyi getirin
...
select * from meslekler  order by maas desc  offset 3 limit 3 ;--ilk 3 kisiyi atladi offset ile.
--tek 4. kisiyi verseydi?
select * from meslekler  order by maas desc  offset 3 limit 1;
...
---2.yol Right Join
SELECT film_name,category,actor_name
FROM aktorler as A RIGTH JOIN filmler as B
ON A.film_id=B.film_id;
...
FROM aktorler as A LEFT JOIN filmler as B
ON A.film_id=B.film_id;
--2.YOL Right join
SELECT actor_name,film_name
FROM filmler as A right JOIN aktorler as D
